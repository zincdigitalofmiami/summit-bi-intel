...
import { calculateTotal, encodeSignToken, type ProposalLineItem, type ProposalRecord } from "@/lib/proposals";
	...
	const sendProposalEmail = async (p: ProposalRecord) => {
		...
		try {
			...
			if (result.success) {
				...
				alert(
					`⚠️ Email service unavailable, but you can still use the sign link:\n${result.signUrl}`,
				);
			}
		} catch (error) {
